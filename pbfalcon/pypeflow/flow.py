...
def task_bam2fasta(self):
    ...
    bash = """
python -m pbfalcon.mains.run_filterbam {i_dataset_fn} {f_dataset_fn} '{filters}'
python -m pbfalcon.mains.run_bam2fasta {f_dataset_fn} {o_fasta_fn}
""".format(**locals())
...
def task_gc_scatter(self):
    ...
    bash = """
python -m pbfalcon.mains.run_gc_scatter \
        {alignmentset_fn} \
        {referenceset_fn} \
        {chunks_fofn_fn}
""".format(**locals())
...
def task_gc_gather(self):
    ...
    bash = r"""
python -m pbfalcon.mains.run_gc_gather \
        {fasta_ds_fofn_fn} \
        {fastq_fofn_fn} \
        {ds_out_fn} \
        {fastq_out_fn}
...
def task_genomic_consensus(self):
    ...
    bash = """
python -m pbfalcon.mains.run_variantCaller --log-level DEBUG --options '{options}' \
        {alignmentset_fn} \
        {referenceset_fn} \
        {polished_fastq_fn} \
        {variants_gff_fn} \
